...
<div class="space-y-6" x-data="{ openDetailModal: false, modalData: {} }">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
        ...
        <div class="bg-white p-4 sm:p-5 rounded-2xl border-2 border-gray-200 shadow-sm hover:border-blue-300 hover:shadow-md transition duration-300 flex items-center justify-between group">
            <div class="space-y-1">
                <span class="text-[10px] sm:text-[11px] font-bold text-gray-500 uppercase tracking-wider block">Buku Dipinjam</span>
                <span class="text-xl sm:text-3xl font-black text-gray-900 group-hover:text-blue-700 transition">{{ $activeLoans->count() }}</span>
                <span class="text-[10px] text-gray-400 font-medium block">Aktif Ditangan</span>
            </div>
            <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-blue-50 border border-blue-100 text-blue-700 flex items-center justify-center shrink-0 shadow-2xs group-hover:scale-110 transition duration-300">
                <svg class="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>
            </div>
        </div>

        <!-- Card 2: Mendekati Tempo -->
        <div class="bg-white p-4 sm:p-5 rounded-2xl border-2 border-gray-200 shadow-sm hover:border-amber-300 hover:shadow-md transition duration-300 flex items-center justify-between group">
            <div class="space-y-1">
                <span class="text-[10px] sm:text-[11px] font-bold text-gray-500 uppercase tracking-wider block">Mendekati Tempo</span>
                <span class="text-xl sm:text-3xl font-black text-amber-600 block">{{ $nearingDueLoans->count() }}</span>
                <span class="text-[10px] text-amber-500 font-medium block">Perlu Perpanjangan</span>
            </div>
            <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-amber-50 border border-amber-100 text-amber-700 flex items-center justify-center shrink-0 shadow-2xs group-hover:scale-110 transition duration-300">
        ...
        <div class="bg-white p-4 sm:p-5 rounded-2xl border-2 border-gray-200 shadow-sm hover:border-rose-300 hover:shadow-md transition duration-300 flex items-center justify-between group">
            <div class="space-y-1">
                <span class="text-[10px] sm:text-[11px] font-bold text-gray-500 uppercase tracking-wider block">Total Denda Aktif</span>
                <span class="text-lg sm:text-2xl font-black text-brand-700 block">Rp {{ number_format($totalFines, 0, ',', '.') }}</span>
                <span class="text-[10px] text-gray-400 font-medium block">Tunggakan Pengembalian</span>
            </div>
            <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-rose-50 border border-rose-100 text-rose-700 flex items-center justify-center shrink-0 shadow-2xs group-hover:scale-110 transition duration-300">
                <svg class="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            </div>
        </div>

        <!-- Card 4: Reservasi Aktif -->
        <div class="bg-white p-4 sm:p-5 rounded-2xl border-2 border-gray-200 shadow-sm hover:border-purple-300 hover:shadow-md transition duration-300 flex items-center justify-between group">
            <div class="space-y-1">
                <span class="text-[10px] sm:text-[11px] font-bold text-gray-500 uppercase tracking-wider block">Reservasi Aktif</span>
                <span class="text-xl sm:text-3xl font-black text-purple-700 block">{{ $activeReservations->count() }}</span>
                <span class="text-[10px] text-gray-400 font-medium block">Antrean Diproses</span>
            </div>
            <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-purple-50 border border-purple-100 text-purple-700 flex items-center justify-center shrink-0 shadow-2xs group-hover:scale-110 transition duration-300">
    ...
    <div class="bg-white rounded-2xl border-2 border-gray-200 shadow-sm overflow-hidden">
        <div class="p-4 sm:p-5 border-b-2 border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
                <h2 class="text-xs sm:text-sm font-black text-gray-900">Buku Sedang Dipinjam Saya</h2>
                <p class="text-[10px] sm:text-[11px] text-gray-500 mt-0.5">Daftar buku fisik yang saat ini berada dalam masa pinjam Anda</p>
            </div>
            <a href="{{ route('mahasiswa.peminjaman') }}" class="text-xs font-bold text-brand-700 hover:text-brand-800 transition shrink-0">Lihat Semua Peminjaman &rarr;</a>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse text-xs">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-200 text-gray-500 uppercase tracking-wider font-extrabold">
                        <th class="py-3.5 px-4 sm:px-5 font-bold">Kode TRX</th>
                        <th class="py-3.5 px-4 sm:px-5 font-bold">Judul Buku</th>
                        <th class="py-3.5 px-4 sm:px-5 font-bold">Tanggal Pinjam</th>
                        <th class="py-3.5 px-4 sm:px-5 font-bold">Jatuh Tempo</th>
                        <th class="py-3.5 px-4 sm:px-5 font-bold">Status Tempo</th>
                        <th class="py-3.5 px-4 sm:px-5 font-bold text-right">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 text-gray-700 font-medium">
                    @forelse($activeLoans as $loan)
                        ...
                        <tr class="hover:bg-gray-50/70 transition">
                            <td class="py-3.5 px-4 sm:px-5 font-mono font-bold text-gray-900 whitespace-nowrap">{{ $loan->kode_peminjaman }}</td>
                            <td class="py-3.5 px-4 sm:px-5 font-bold text-gray-900">{{ $loan->buku->judul ?? '-' }}</td>
                            <td class="py-3.5 px-4 sm:px-5 text-gray-600 whitespace-nowrap">{{ $loan->tanggal_pinjam }}</td>
                            <td class="py-3.5 px-4 sm:px-5 text-gray-600 font-bold whitespace-nowrap">{{ $loan->tanggal_jatuh_tempo }}</td>
    ...
    <div class="bg-white rounded-3xl border-2 border-gray-200 shadow-sm p-4 sm:p-6 space-y-6">
        
        <!-- Header & Responsive Search Filter Bar -->
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-4 border-b-2 border-gray-100">
            <div>
                ...
                <p class="text-[11px] sm:text-xs text-gray-500 mt-0.5">Cari judul buku dan ajukan booking / peminjaman langsung dari dashboard Anda.</p>
            </div>

            <!-- Responsive Search Form -->
            <form action="{{ route('mahasiswa.dashboard') }}" method="GET" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 w-full lg:w-auto">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari judul, ISBN, penulis..."
                    class="w-full sm:w-48 lg:w-64 px-3.5 py-2 bg-gray-50 border border-gray-300 rounded-xl text-xs focus:ring-2 focus:ring-brand-700">
                
                <select name="kategori_id" onchange="this.form.submit()" class="w-full sm:w-auto px-3 py-2 bg-gray-50 border border-gray-300 rounded-xl text-xs font-semibold focus:ring-2 focus:ring-brand-700">
                ...
                <button type="submit" class="w-full sm:w-auto px-4 py-2 bg-brand-700 hover:bg-brand-800 text-white font-extrabold text-xs rounded-xl transition shadow-2xs text-center justify-center">Cari</button>
            </form>
        </div>

        <!-- Responsive Book Cards Grid -->
