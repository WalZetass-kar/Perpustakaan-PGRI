...
@section('content')
<!-- Print Specific CSS Stylesheet - Exact Physical CR80 Dimensions (85.6mm x 54mm) -->
<style>
    @media print {
        /* Default Single CR80 Card Page Settings */
        @page {
            size: 85.6mm 54mm;
            margin: 0;
        }
        ...
        .single-print-mode #printable-card {
        ...
        .sheet-print-mode .sheet-grid-container {
        ...
        .sheet-print-mode #printable-card {
...
<div class="max-w-xl mx-auto space-y-6" x-data="{ printMode: 'single' }">

    <!-- Action Bar (Print & Upload Photo - Hidden on Print) -->
    <div class="no-print bg-white p-4 sm:p-5 rounded-2xl border-2 border-gray-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
        <div>
            <h2 class="text-sm font-black text-gray-900">Sistem Cetak Kartu Anggota Presisi (CR80)</h2>
            <p class="text-[11px] text-gray-500 mt-0.5">Ukuran cetak otomatis disesuaikan ke standar fisik <strong class="text-gray-900">85.6 mm × 54 mm</strong>.</p>
        </div>
        <div class="flex flex-wrap items-center gap-3 shrink-0">
            <!-- Mode Toggle Option -->
            <select x-model="printMode" class="px-3 py-2 bg-gray-50 border-2 border-gray-200 rounded-xl text-xs font-bold text-gray-800 focus:outline-none focus:ring-2 focus:ring-red-700">
            ...
            <button @click="
                ...
                window.print();
            " class="px-4 py-2.5 bg-red-700 hover:bg-red-800 text-white font-extrabold text-xs rounded-xl transition shadow-md hover:shadow-lg transform active:scale-95 flex items-center gap-2">
    ...
    <!-- OFFICIAL INSTITUTIONAL LIBRARY CARD FRONT SIDE (Standard Responsive Screen View, 85.6mm Print View) -->
    <div class="single-print-wrapper">
        <div class="library-card w-full max-w-[540px] mx-auto bg-white rounded-2xl border-2 border-red-700 shadow-xl overflow-hidden flex flex-col justify-between relative" id="printable-card">
            ...
            <div class="p-4 sm:p-5 bg-white flex items-center justify-between gap-4 flex-1">
                ...
                <!-- Area 1: Student Photo (Enlarged Pas Foto 4:5 Ratio) -->
                <div class="shrink-0">
                    <div class="card-photo-frame w-32 h-40 sm:w-34 sm:h-42 bg-gray-50 border-2 border-red-700 rounded-xl overflow-hidden flex flex-col items-center justify-center shadow-xs relative group">
                ...
                <div class="card-info-container flex-1 min-w-0 space-y-2.5 text-[10.5px]">
                    <div>
                        <span class="card-label text-[8.5px] font-extrabold text-gray-500 uppercase tracking-wider block leading-none">NAMA LENGKAP SISWA</span>
                    ...
                    <div class="grid grid-cols-2 gap-2 border-t border-b border-gray-100 py-2">
                        <div>
                            <span class="card-label text-[8.5px] font-extrabold text-gray-500 uppercase tracking-wider block leading-none">NISN / NIS</span>
                            <span class="card-data-val font-mono text-gray-800 font-extrabold text-[10.5px] block truncate mt-0.5">{{ $anggota->nim ?? '0877667687' }}</span>
                        </div>
                        <div>
                            <span class="card-label text-[8.5px] font-extrabold text-gray-500 uppercase tracking-wider block leading-none">ID ANGGOTA</span>
                            <span class="card-data-val font-mono text-red-700 font-extrabold text-[10.5px] block truncate mt-0.5">{{ $anggota->nomor_anggota ?? 'LIB-2026-005' }}</span>
                        </div>
                    </div>

                    <div>
                        <span class="card-label text-[8.5px] font-extrabold text-gray-500 uppercase tracking-wider block leading-none">PROGRAM / JURUSAN</span>
                        <span class="card-data-val text-gray-900 font-bold text-[10.5px] block truncate leading-tight mt-0.5">{{ $anggota->program_studi ?? 'Teknik Komputer & Jaringan (TKJ)' }}</span>
                    </div>
                </div>
                ...
                <div class="card-qr-box shrink-0 flex flex-col items-center justify-center p-2.5 bg-white rounded-xl border border-gray-200 shadow-2xs text-center space-y-1">
                    <div class="card-qr-img-wrap w-18 h-18 bg-white p-0.5 rounded-lg border border-gray-200 flex items-center justify-center">
                        <img src="{{ $qrApiUrl }}" alt="QR Code Verifikasi" class="w-full h-full object-contain">
                    </div>
                    <span class="card-qr-label text-[8.5px] font-black text-emerald-600 block uppercase tracking-wider">VALID QR</span>
                    <span class="card-qr-subtext text-[7.5px] font-bold text-gray-400 block max-w-[85px] leading-tight">Scan untuk verifikasi anggota</span>
                </div>
