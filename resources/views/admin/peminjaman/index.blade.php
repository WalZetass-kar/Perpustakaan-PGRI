    ...
    <div class="bg-white p-4 sm:p-5 rounded-2xl border-2 border-gray-200 shadow-sm"
        ...
        <div class="hidden sm:flex sm:flex-row items-center justify-between gap-3">
            <div class="flex items-center gap-2 min-w-0">
                <a href="{{ route('admin.peminjaman') }}" class="px-3 py-1.5 rounded-xl text-xs font-bold transition {{ request('filter') !== 'terlambat' ? 'bg-brand-700 text-white shadow-sm' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                    Semua Aktif ({{ $totalActive ?? $activeLoans->total() }})
                </a>
                <a href="{{ route('admin.peminjaman', ['filter' => 'terlambat']) }}" class="px-3 py-1.5 rounded-xl text-xs font-bold transition {{ request('filter') === 'terlambat' ? 'bg-rose-700 text-white shadow-sm' : 'bg-rose-50 text-rose-700 hover:bg-rose-100 border border-rose-200' }}">
        ...
        <div class="sm:hidden space-y-3">
            <div class="flex items-center gap-2">
                <a href="{{ route('admin.peminjaman') }}" class="flex-1 text-center py-1.5 rounded-xl text-xs font-bold transition {{ request('filter') !== 'terlambat' ? 'bg-brand-700 text-white shadow-sm' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                    Semua ({{ $totalActive ?? $activeLoans->total() }})
                </a>
                <a href="{{ route('admin.peminjaman', ['filter' => 'terlambat']) }}" class="flex-1 text-center py-1.5 rounded-xl text-xs font-bold transition {{ request('filter') === 'terlambat' ? 'bg-rose-700 text-white shadow-sm' : 'bg-rose-50 text-rose-700 hover:bg-rose-100 border border-rose-200' }}">
                    Terlambat ({{ $totalOverdue ?? 0 }})
                </a>
            </div>
