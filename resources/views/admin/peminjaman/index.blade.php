...
<div class="space-y-5" x-data="{ openAddModal: false, selectedBookQty: 1 }" x-init="openAddModal = false">
    ...
    <div class="bg-white p-4 sm:p-5 rounded-2xl border-2 border-gray-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2 w-full sm:w-auto">
            <form action="{{ route('admin.peminjaman') }}" method="GET" class="relative flex-1 sm:w-72">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari kode transaksi, peminjam, judul..." 
            ...
            <table class="w-full text-left border-collapse text-xs">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-200 text-gray-500 uppercase tracking-wider font-extrabold">
                        ...
                        <th class="py-3 px-4 font-bold">Peminjam (Anggota)</th>
                ...
                <tbody class="divide-y divide-gray-100 text-gray-700 font-medium">
                    @forelse($activeLoans as $loan)
                        <tr class="hover:bg-gray-50/70 transition">
                            ...
                            <td class="py-3 px-4">
                                <p class="font-bold text-gray-900">{{ $loan->user->name ?? '-' }}</p>
                                <p class="text-[10px] text-gray-500 font-mono">{{ $loan->user->anggota->nim ?? $loan->user->email }}</p>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="py-8 text-center text-gray-400 font-medium">Tidak ada buku yang sedang dipinjam saat ini. Semua buku berada di rak.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div x-show="openAddModal" @click.self="openAddModal = false" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-950/60 backdrop-blur-sm p-3 sm:p-4 overflow-y-auto" x-cloak>
        <div @click.stop class="bg-white rounded-2xl max-w-md w-full max-h-[90vh] flex flex-col shadow-2xl border-2 border-gray-200 overflow-hidden transform transition-all my-auto">
            <div class="px-5 py-3.5 border-b border-gray-100 flex items-center justify-between shrink-0 bg-gray-50/70">
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] text-gray-500">Peminjaman dan pengembalian di hari yang sama</p>
                    </div>
                </div>
            ...
            <form action="{{ route('admin.peminjaman.store') }}" method="POST" class="flex-1 overflow-y-auto p-4 sm:p-5 space-y-3 text-xs">
                @csrf
                <div>
                    <label class="block font-bold text-gray-700 mb-1">Pilih Anggota / Peminjam <span class="text-rose-500">*</span></label>
                    <select name="user_id" required class="w-full px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-1.5 focus:ring-brand-700 focus:bg-white focus:outline-none font-medium">
                        <option value="">-- Pilih Anggota Perpustakaan --</option>
                        @foreach($usersList as $usr)
                            <option value="{{ $usr->id }}">
                                {{ $usr->name }} ({{ $usr->anggota->nim ?? $usr->email }})
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="buku_id" required class="w-full px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-1.5 focus:ring-brand-700 focus:bg-white focus:outline-none font-medium">
                ...
                <div>
                    ...
                    <input type="number" name="jumlah" value="1" required min="1" max="10" class="w-full px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-1.5 focus:ring-brand-700 focus:bg-white focus:outline-none font-bold text-brand-700">
                </div>
                ...
                <div class="p-3 rounded-xl bg-amber-50 border border-amber-200 text-amber-900 text-[11px] leading-relaxed">
                    <p class="font-bold flex items-center gap-1.5">
                        ...
                        <span>Aturan Operasional Hari Ini:</span>
                    </p>
