...
@section('page_heading', 'Master Data Penerbit Buku')
...
<div class="space-y-5" x-data="{ openAddModal: false, openEditModal: false, editData: {} }" x-init="openAddModal = false; openEditModal = false; editData = {}">
    ...
    <div class="bg-white p-4 sm:p-5 rounded-2xl border-2 border-gray-200 shadow-sm"
        ...
        <div class="flex items-center justify-between gap-2 sm:gap-3">
            <div class="flex items-center gap-2 min-w-0">
                ...
                <form action="{{ route('admin.penerbit') }}" method="GET" class="hidden sm:block relative sm:w-64">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama penerbit atau kota..."
            ...
            <button @click="openAddModal = true" class="px-3.5 py-1.5 bg-brand-700 hover:bg-brand-800 text-white text-xs font-extrabold rounded-xl transition duration-200 shadow-sm flex items-center gap-1.5 shrink-0">
                ...
                <span>Penerbit Baru</span>
            </button>
        </div>

        {{-- Search bar mobile: collapsible, muncul di baris baru di bawah --}}
        <form action="{{ route('admin.penerbit') }}" method="GET" x-show="searchOpen" x-cloak
    ...
    <div class="bg-white rounded-2xl border-2 border-gray-200 shadow-sm overflow-hidden">
        <div class="hidden lg:block overflow-x-auto">
            <table class="w-full text-left border-collapse text-xs">
                ...
                <tbody class="divide-y divide-gray-100 text-gray-700 font-medium">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="py-8 text-center text-gray-400 font-medium">Belum ada data penerbit terdaftar.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="lg:hidden divide-y divide-gray-100">
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex items-start justify-between gap-2">
                                <p class="font-bold text-gray-900 text-xs leading-snug pt-1.5">{{ $p->nama }}</p>
                            ...
                            <p class="flex items-center gap-1.5 text-[11px] text-gray-500 font-medium mt-1">
                                <i class="fa-solid fa-location-dot text-gray-300 text-[10px]"></i>
                                {{ $p->kota ?? 'Kota tidak diketahui' }}
                            </p>
            ...
            @empty
                <div class="py-10 text-center text-gray-400 font-medium text-xs">Belum ada data penerbit terdaftar.</div>
            @endforelse
        </div>
    ...
    <div x-show="openAddModal" @click.self="openAddModal = false" class="fixed inset-0 z-[100] !mt-0 flex items-center justify-center bg-gray-950/60 backdrop-blur-sm p-3 sm:p-4 overflow-y-auto"          x-transition:enter="transition ease-out duration-300"
        ...
        <div @click.stop class="bg-white rounded-2xl max-w-md w-full max-h-[90vh] flex flex-col shadow-2xl border-2 border-gray-200 overflow-hidden transform transition-all my-auto">
            <div class="px-5 py-3.5 border-b border-gray-100 flex items-center justify-between shrink-0 bg-gray-50/70">
                <h3 class="text-sm font-black text-gray-900">Tambah Master Penerbit</h3>
            ...
            <form action="{{ route('admin.penerbit.store') }}" method="POST" class="flex-1 overflow-y-auto p-4 sm:p-5 space-y-3 text-xs">
                @csrf
                <div>
                    <label class="block font-bold text-gray-700 mb-1">Nama Penerbit <span class="text-rose-500">*</span></label>
                    <input type="text" name="nama" required placeholder="Contoh: Penerbit Andi, Erlangga, Gramedia" class="w-full px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-1.5 focus:ring-brand-700 focus:bg-white focus:outline-none font-medium">
                </div>
                <div>
                    <label class="block font-bold text-gray-700 mb-1">Kota Asal (Opsional)</label>
                    <input type="text" name="kota" placeholder="Contoh: Jakarta, Yogyakarta, Bandung" class="w-full px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-1.5 focus:ring-brand-700 focus:bg-white focus:outline-none font-medium">
                </div>
                <div class="pt-3 border-t border-gray-100 flex justify-end gap-2 shrink-0">
                    ...
                    <button type="submit" class="px-5 py-2 bg-brand-700 text-white font-extrabold rounded-xl hover:bg-brand-800 text-xs">Simpan Penerbit</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div x-show="openEditModal" @click.self="openEditModal = false" class="fixed inset-0 z-[100] !mt-0 flex items-center justify-center bg-gray-950/60 backdrop-blur-sm p-3 sm:p-4 overflow-y-auto"          x-transition:enter="transition ease-out duration-300"
        ...
        <div @click.stop class="bg-white rounded-2xl max-w-md w-full max-h-[90vh] flex flex-col shadow-2xl border-2 border-gray-200 overflow-hidden transform transition-all my-auto">
            <div class="px-5 py-3.5 border-b border-gray-100 flex items-center justify-between shrink-0 bg-amber-50/50">
                <h3 class="text-sm font-black text-gray-900">Edit Data Penerbit</h3>
            ...
            <form :action="'{{ url('/admin/penerbit/update') }}/' + editData.id" method="POST" class="flex-1 overflow-y-auto p-4 sm:p-5 space-y-3 text-xs">
                @csrf
                <div>
                    <label class="block font-bold text-gray-700 mb-1">Nama Penerbit <span class="text-rose-500">*</span></label>
                ...
                <div>
                    <label class="block font-bold text-gray-700 mb-1">Kota Asal (Opsional)</label>
                    <input type="text" name="kota" x-model="editData.kota" class="w-full px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-1.5 focus:ring-brand-700 focus:bg-white focus:outline-none font-medium">
                </div>
                <div class="pt-3 border-t border-gray-100 flex justify-end gap-2 shrink-0">
