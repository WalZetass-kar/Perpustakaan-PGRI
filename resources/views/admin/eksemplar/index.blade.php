...
<div class="space-y-5" x-data="{ openAddModal: false, openEditModal: false, editData: {} }" x-init="openAddModal = false; openEditModal = false; editData = {}">
    ...
    <!-- Information Banner -->
    <div class="p-4 sm:p-5 rounded-2xl bg-gradient-to-r from-brand-900 to-brand-800 text-white border-2 border-brand-950 shadow-md flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
        <div class="space-y-1">
            ...
            <p class="text-xs text-red-100 max-w-3xl leading-relaxed font-medium">
                Setiap 1 Master Judul Buku dapat memiliki banyak fisik buku (Eksemplar) dengan **Barcode ID unik** untuk scanning sirkulasi.
            </p>
        </div>
        <div class="flex items-center gap-2 shrink-0">
            <a href="{{ route('admin.eksemplar.cetak_barcode') }}" target="_blank" class="px-3.5 py-2 bg-amber-400 hover:bg-amber-300 text-brand-950 font-extrabold text-xs rounded-xl transition shadow-md flex items-center gap-1.5">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/></svg>
                <span>Cetak Barcode (A4)</span>
            </a>
            <button @click="openAddModal = true" class="px-3.5 py-2 bg-white text-brand-700 font-extrabold text-xs rounded-xl hover:bg-gray-100 transition shadow-md hover:shadow-lg transform active:scale-95 shrink-0 flex items-center gap-1.5">
            ...
            <table class="w-full text-left border-collapse text-xs">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-200 text-gray-500 uppercase tracking-wider font-extrabold">
                        ...
                        <th class="py-3 px-4 font-bold">Master Judul Buku</th>
                        <th class="py-3 px-4 font-bold">Posisi Rak</th>
                        <th class="py-3 px-4 font-bold">Kondisi Fisik</th>
                        <th class="py-3 px-4 font-bold">Status Sirkulasi</th>
                ...
                <tbody class="divide-y divide-gray-100 text-gray-700 font-medium">
                    @foreach($eksemplarList as $ex)
                        <tr class="hover:bg-gray-50/70 transition">
                            <td class="py-3 px-4 font-mono font-black text-brand-700">{{ $ex->barcode }}</td>
                            <td class="py-3 px-4 font-mono text-gray-900 font-bold">{{ $ex->kode_eksemplar }}</td>
                            <td class="py-3 px-4 font-bold text-gray-900 max-w-xs truncate">{{ $ex->buku->judul ?? '-' }}</td>
                            <td class="py-3 px-4">
                            ...
                            <td class="py-3 px-4">
                                @if($ex->status === 'tersedia')
                                    <span class="px-2.5 py-0.5 rounded-lg text-[10px] font-extrabold bg-emerald-50 text-emerald-700 border border-emerald-200 uppercase">Tersedia</span>
                                @elseif($ex->status === 'dipinjam')
                                    <span class="px-2.5 py-0.5 rounded-lg text-[10px] font-extrabold bg-blue-50 text-blue-700 border border-blue-200 uppercase">Dipinjam</span>
                                @else
                                    <span class="px-2.5 py-0.5 rounded-lg text-[10px] font-extrabold bg-rose-50 text-rose-700 border border-rose-200 uppercase">{{ $ex->status }}</span>
                                @endif
                            </td>
    ...
    <!-- Modal Form Tambah Eksemplar (Compact & Fully Visible) -->
        ...
        <div @click.stop class="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] flex flex-col shadow-2xl border-2 border-gray-200 overflow-hidden transform transition-all my-auto">
            ...
            <div class="px-5 py-3.5 border-b border-gray-100 flex items-center justify-between shrink-0 bg-gray-50/70">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <h3 class="text-sm font-black text-gray-900">Registrasi Eksemplar Baru</h3>
                        <p class="text-[10px] text-gray-500">Daftarkan fisik buku baru ke dalam koleksi</p>
                    </div>
                </div>
            ...
            <form action="{{ route('admin.eksemplar.store') }}" method="POST" class="flex-1 overflow-y-auto p-4 sm:p-5 space-y-3 text-xs">
                @csrf
                <div>
                    <label class="block font-bold text-gray-700 mb-1">Pilih Master Buku <span class="text-rose-500">*</span></label>
                    <select name="buku_id" required class="w-full px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-1.5 focus:ring-brand-700 focus:border-brand-700 focus:bg-white focus:outline-none font-medium">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block font-bold text-gray-700 mb-1">Kode Eksemplar <span class="text-rose-500">*</span></label>
                        <input type="text" name="kode_eksemplar" required placeholder="EX-WEB-003" class="w-full px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-1.5 focus:ring-brand-700 focus:border-brand-700 focus:bg-white focus:outline-none font-medium">
                    </div>
                    <div>
                        <label class="block font-bold text-gray-700 mb-1">Barcode ID <span class="text-rose-500">*</span></label>
                        <input type="text" name="barcode" required placeholder="BC882003" class="w-full px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-1.5 focus:ring-brand-700 focus:border-brand-700 focus:bg-white focus:outline-none font-medium">
                    </div>
                </div>

                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block font-bold text-gray-700 mb-1">Kondisi Buku <span class="text-rose-500">*</span></label>
                        <select name="kondisi" required class="w-full px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-1.5 focus:ring-brand-700 focus:border-brand-700 focus:bg-white focus:outline-none font-medium">
                    ...
                    <div>
                        <label class="block font-bold text-gray-700 mb-1">Pilih Rak <span class="text-rose-500">*</span></label>
                ...
                <div class="pt-3 border-t border-gray-100 flex justify-end gap-2 shrink-0">
                    ...
                    <button type="submit" class="px-5 py-2 bg-brand-700 text-white font-extrabold rounded-xl hover:bg-brand-800 transition shadow-md hover:shadow-lg transform active:scale-95 text-xs">Simpan Eksemplar</button>
                </div>
            </form>
        </div>
    </div>

    <!-- Modal Form Edit Eksemplar (Compact & Fully Visible) -->
        ...
        <div @click.stop class="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] flex flex-col shadow-2xl border-2 border-gray-200 overflow-hidden transform transition-all my-auto">
            ...
            <div class="px-5 py-3.5 border-b border-gray-100 flex items-center justify-between shrink-0 bg-amber-50/50">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <h3 class="text-sm font-black text-gray-900">Edit Data Eksemplar</h3>
                        <p class="text-[10px] text-gray-500">Perbarui kondisi fisik & status sirkulasi</p>
                    </div>
                </div>
            ...
            <!-- Modal Body (Scrollable Form) -->
            <form :action="'{{ url('/admin/eksemplar/update') }}/' + editData.id" method="POST" class="flex-1 overflow-y-auto p-4 sm:p-5 space-y-3 text-xs">
                @csrf
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block font-bold text-gray-700 mb-1">Kode Eksemplar <span class="text-rose-500">*</span></label>
                        <input type="text" name="kode_eksemplar" x-model="editData.kode_eksemplar" required class="w-full px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-1.5 focus:ring-brand-700 focus:border-brand-700 focus:bg-white focus:outline-none font-medium">
                    </div>
                    <div>
                        <label class="block font-bold text-gray-700 mb-1">Barcode ID <span class="text-rose-500">*</span></label>
                        <input type="text" name="barcode" x-model="editData.barcode" required class="w-full px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-1.5 focus:ring-brand-700 focus:border-brand-700 focus:bg-white focus:outline-none font-medium">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-3 gap-3">
                    <div>
                        <label class="block font-bold text-gray-700 mb-1">Kondisi Buku <span class="text-rose-500">*</span></label>
                        <select name="kondisi" x-model="editData.kondisi" required class="w-full px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-1.5 focus:ring-brand-700 focus:border-brand-700 focus:bg-white focus:outline-none font-medium">
                            <option value="baik">Baik</option>
                            <option value="rusak_ringan">Rusak Ringan</option>
                            <option value="rusak_berat">Rusak Berat</option>
                        </select>
                    ...
                    <div>
                        <label class="block font-bold text-gray-700 mb-1">Status <span class="text-rose-500">*</span></label>
