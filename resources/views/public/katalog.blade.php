...
<div class="space-y-6 pb-12" x-data="{ 
    ...
    <div class="block lg:hidden">
        ...
        @endphp
        <button @click="mobileFilterOpen = true" class="w-full p-3 bg-white border-2 border-gray-200 rounded-2xl font-black text-xs text-gray-800 flex items-center justify-between shadow-2xs">
            <div class="flex items-center gap-2">
                <svg class="w-4 h-4 text-brand-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"/></svg>
                <span>Filter Katalog</span>
                @if($activeFilterCount > 0)
                    <span class="px-2 py-0.5 rounded-full bg-brand-700 text-white text-[10px] font-black">{{ $activeFilterCount }}</span>
                @endif
            </div>
            <span class="text-gray-400 text-xs">Ketuk untuk Atur &raquo;</span>
        </button>
    </div>

    <!-- MAIN GRID CONTAINER (FILTER SIDEBAR & CATALOG SHOWCASE) -->
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-7 items-start">
        
        <!-- 3. COMPACT FILTER SIDEBAR (Desktop) -->
        <div class="hidden lg:block lg:col-span-1">
            <form action="{{ route('katalog') }}" method="GET" class="bg-white p-5 rounded-2xl border-2 border-gray-200 shadow-2xs space-y-3.5 text-xs sticky top-4">
                @if(request('search')) <input type="hidden" name="search" value="{{ request('search') }}"> @endif
                @if(request('sort')) <input type="hidden" name="sort" value="{{ request('sort') }}"> @endif

                <div class="flex items-center justify-between border-b border-gray-100 pb-2.5">
                    <h2 class="font-black text-gray-900 uppercase tracking-wider text-xs flex items-center gap-2">
                        <svg class="w-4 h-4 text-brand-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"/></svg>
                        <span>FILTER KATALOG</span>
                    </h2>
                    @if(request()->anyFilled(['kategori_id', 'penulis_id', 'rak_id', 'tahun', 'status']))
                        <a href="{{ route('katalog', request()->only('search', 'sort')) }}" class="text-[10px] font-bold text-rose-600 hover:underline">Reset</a>
                    @endif
                </div>

                <div>
                    <label class="block font-extrabold text-gray-700 mb-1">Kategori</label>
                    <select name="kategori_id" class="w-full px-3 py-2 bg-gray-50 border-2 border-gray-200 rounded-xl font-bold text-gray-800 focus:ring-2 focus:ring-brand-700 focus:bg-white focus:outline-none transition">
                ...
                <div>
                    ...
                    <select name="penulis_id" class="w-full px-3 py-2 bg-gray-50 border-2 border-gray-200 rounded-xl font-medium text-gray-800 focus:ring-2 focus:ring-brand-700 focus:bg-white focus:outline-none transition">
                ...
                <div>
                    ...
                    <select name="rak_id" class="w-full px-3 py-2 bg-gray-50 border-2 border-gray-200 rounded-xl font-medium text-gray-800 focus:ring-2 focus:ring-brand-700 focus:bg-white focus:outline-none transition">
                ...
                <div>
                    ...
                    <select name="tahun" class="w-full px-3 py-2 bg-gray-50 border-2 border-gray-200 rounded-xl font-medium text-gray-800 focus:ring-2 focus:ring-brand-700 focus:bg-white focus:outline-none transition">
                ...
                <div>
                    ...
                    <select name="status" class="w-full px-3 py-2 bg-gray-50 border-2 border-gray-200 rounded-xl font-bold text-gray-800 focus:ring-2 focus:ring-brand-700 focus:bg-white focus:outline-none transition">
                ...
                <div class="pt-2 flex items-center gap-2">
                    <button type="submit" class="w-full py-2.5 bg-brand-700 hover:bg-brand-800 text-white font-extrabold text-xs rounded-xl transition shadow-sm transform active:scale-95">
                        Terapkan Filter
                    </button>
                    <a href="{{ route('katalog', request()->only('search', 'sort')) }}" class="px-3 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold text-xs rounded-xl transition shrink-0">
                        Reset
                    </a>
                </div>
            </form>
        ...
        <div class="lg:col-span-3 space-y-5" x-data="{ isLoading: true }" x-init="setTimeout(() => isLoading = false, 200)">
            
            <!-- 5 & 6. RESULT SUMMARY, VIEW SWITCHER & SORTING BAR -->
            <div class="bg-white p-4 rounded-2xl border-2 border-gray-200 shadow-2xs flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
                <!-- Result Count Text -->
                <div>
                    @if(request('search'))
                        <h3 class="font-extrabold text-gray-900">Hasil pencarian untuk "<span class="text-brand-700">{{ request('search') }}</span>" — {{ $buku->total() }} buku ditemukan</h3>
                    @else
                        <h3 class="font-extrabold text-gray-900">{{ $buku->total() }} Buku Ditemukan</h3>
                    @endif
                </div>

                <!-- Grid/List Switcher & Sort Selector -->
            ...
            <div x-show="isLoading" class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5 animate-pulse" x-cloak>
                @for($i=0; $i<6; $i++)
            ...
            <!-- 7 & 8. BOOK SHOWCASE GRID & LIST VIEWS -->
    ...
    <div x-show="mobileFilterOpen" @click.self="mobileFilterOpen = false" class="fixed inset-0 z-50 flex justify-end bg-gray-950/60 backdrop-blur-xs"
         ...
         x-cloak>
        <div class="bg-white w-full max-w-xs h-full p-6 space-y-4 shadow-2xl overflow-y-auto" @click.stop>
            <div class="flex items-center justify-between border-b-2 border-gray-100 pb-3">
                <h3 class="font-black text-gray-900 text-sm">Filter Katalog OPAC</h3>
                <button @click="mobileFilterOpen = false" class="w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-600 font-bold">&times;</button>
            </div>
            ...
            <form action="{{ route('katalog') }}" method="GET" class="space-y-4 text-xs">
                ...
                <div>
                    <label class="block font-extrabold text-gray-700 mb-1">Kategori Kejuruan</label>
                    <select name="kategori_id" class="w-full px-3 py-2.5 bg-gray-50 border-2 border-gray-200 rounded-xl font-bold text-gray-800">
                        <option value="">Semua Kategori</option>
                        @foreach($kategori_list as $kat)
                            <option value="{{ $kat->id }}" {{ request('kategori_id') == $kat->id ? 'selected' : '' }}>{{ $kat->nama }}</option>
                        @endforeach
                ...
                <div>
                    <label class="block font-extrabold text-gray-700 mb-1">Penulis Buku</label>
                    <select name="penulis_id" class="w-full px-3 py-2.5 bg-gray-50 border-2 border-gray-200 rounded-xl font-medium text-gray-800">
                        <option value="">Semua Penulis</option>
                        @foreach($penulis_list as $pen)
                            <option value="{{ $pen->id }}" {{ request('penulis_id') == $pen->id ? 'selected' : '' }}>{{ $pen->nama }}</option>
                        @endforeach
                ...
                <div>
                    <label class="block font-extrabold text-gray-700 mb-1">Lokasi Rak</label>
                    <select name="rak_id" class="w-full px-3 py-2.5 bg-gray-50 border-2 border-gray-200 rounded-xl font-medium text-gray-800">
                        <option value="">Semua Lokasi Rak</option>
                        @foreach($rak_list as $rk)
                            <option value="{{ $rk->id }}" {{ request('rak_id') == $rk->id ? 'selected' : '' }}>{{ $rk->kode_rak }} ({{ $rk->nama_rak }})</option>
                        @endforeach
                ...
                <div>
                    <label class="block font-extrabold text-gray-700 mb-1">Tahun Terbit</label>
                    <select name="tahun" class="w-full px-3 py-2.5 bg-gray-50 border-2 border-gray-200 rounded-xl font-medium text-gray-800">
                        <option value="">Semua Tahun</option>
                        @foreach($tahun_list as $th)
                            <option value="{{ $th }}" {{ request('tahun') == $th ? 'selected' : '' }}>Tahun {{ $th }}</option>
                        @endforeach
                ...
                <div>
                    <label class="block font-extrabold text-gray-700 mb-1">Status Ketersediaan</label>
                    <select name="status" class="w-full px-3 py-2.5 bg-gray-50 border-2 border-gray-200 rounded-xl font-bold text-gray-800">
                        <option value="">Semua Status</option>
                        <option value="tersedia" {{ request('status') == 'tersedia' ? 'selected' : '' }}>Tersedia Dipinjam</option>
                        <option value="dipinjam" {{ request('status') == 'dipinjam' ? 'selected' : '' }}>Sedang Dipinjam</option>
                    </select>
                ...
                <div class="pt-4 border-t-2 border-gray-100 space-y-2">
                    <button type="submit" class="w-full py-3 bg-brand-700 text-white font-extrabold text-xs rounded-xl hover:bg-brand-800 transition">
                        Terapkan Filter
                    </button>
                    <a href="{{ route('katalog', request()->only('search', 'sort')) }}" class="block w-full text-center py-2.5 bg-gray-100 text-gray-700 font-bold text-xs rounded-xl hover:bg-gray-200 transition">
                        Reset Filter
                    </a>
                </div>
            </form>
